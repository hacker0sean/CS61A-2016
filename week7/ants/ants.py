...
class Ant(Insect):
    ...
    def can_contain(self, another_ant):
        if self.container and (not self.ant) and (not another_ant.container):
            return True
        return False
...
# BEGIN Problem 5A
class WallAnt(Ant):
    """An ant that does nothing each turn. But its armor is 4"""
    name = 'Wall'
    food_cost = 4
    implemented = True
    damage = 0
    ...
    def __init__(self, armor=4):
        """Create an WallAnt with an armor 4"""
...
class ScubaThrower(ThrowerAnt):
    """One kind of ThrowerAnt that can suivive in the water"""
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            while temp:
                if temp.ant:
                    if (not temp.ant in self.inspire_list) or temp.ant.container:
                        if temp.ant.container and temp.ant.ant:
                            if not temp.ant.ant in self.inspire_list:
                                temp.ant.ant.damage *= 2
                                self.inspire_list.append(temp.ant.ant)
                        if not temp.ant in self.inspire_list:
                            temp.ant.damage *= 2
                            self.inspire_list.append(temp.ant)
                temp = temp.exit
        # END Problem 9

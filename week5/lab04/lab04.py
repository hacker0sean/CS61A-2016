...
def distance(city1, city2):
    ...
    5.0
    """
    return sqrt(pow(abs(get_lat(city1) - get_lat(city2)), 2) + pow(abs(get_lon(city1) - get_lon(city2)), 2))
...
def make_move(board, max_rows, max_cols, col, player):
    ...
    if col < 0 or col >= max_cols:
        ...
        rows, board = put_piece(board, max_rows, col, player)
        return (rows, board)
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    number = 0
    for j in range(max_cols):
        if get_piece(board, i, j) == player:
            number += 1
    if number > num_connect:
        return True
    return False
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    diagonal_win = check_win_diagonal(board, max_rows, max_cols, num_connect,
                                      row, col, player)
    return check_win_column(board, max_rows, max_cols, num_connect, col, player) or check_win_row(board, max_rows, max_cols, num_connect, row, player) \
            or diagonal_win

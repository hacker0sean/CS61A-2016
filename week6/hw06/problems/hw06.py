...
def balanced(m):
    ...
    """
    def cal(m):
        compare = [total_weight(end(b)) * length(b) for b in sides(m)]
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, trial):
        nonlocal trial_time, trial_list, balance, lock
        if lock == True:
            return "Your account is locked. Attempts: " + str(trial_list)
        if trial != password:
            trial_time = trial_time + 1
            if trial_time >= locked_time:
                lock = True
...
def make_joint(withdraw, old_password, new_password):
    ...
    "Your account is locked. Attempts: ['my', 'secret', 'password']"
    """
    password_list = [old_password]
    def joint(amount, trial):
        if trial in password_list:
            return withdraw(amount, old_password)
        else:
            return withdraw(amount, trial)
